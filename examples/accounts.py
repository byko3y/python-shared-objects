...
if len(sys.argv) != 3 or sys.argv[1] != 'worker':
    ...
    pso.root().worker_count = 1
    if len(sys.argv) >= 2:
        pso.root().worker_count = int(sys.argv[1])

    pso.root().accounts = {f'client{i}': Account(i, random.randrange(1000)) for i in range(number_of_accounts)}
    #original_values = [client.balance for client in pso.root().accounts.values()]
    original_values = [pso.root().accounts[f'client{i}'].balance for i in range(number_of_accounts)]
    ...
    new_values = [pso.root().accounts[f'client{i}'].balance for i in range(number_of_accounts)]
    ...
    for i in range(100*1000 // pso.root().worker_count):
        source = f'client{random.randrange(number_of_accounts)}'
        target = f'client{random.randrange(number_of_accounts)}'
